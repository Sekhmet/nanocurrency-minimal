...
export function parseAddress(address: {}): ParseAddressResult {
  ...
  if (!checkString(address) || !/^(xrb_|nano_)[13][13-9a-km-uw-z]{59}$/.test(address as string)) {
    return invalid
  }
  ...
  const publicKeyBytes = decodeNanoBase32((address as string).substr(prefixLength, 52))
  const checksumBytes = decodeNanoBase32((address as string).substr(prefixLength + 52))
